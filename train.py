...
from data import *
from data.VisDrone import VisDrone
...
parser.add_argument('--dataset', type=str, default='VOC', choices=['VOC', 'COCO', 'VisDrone'], help='name of dataset used')
...
def train():   
    ...
    while True:
        for input, loc, label in dataloader:
            ...
            output = open('VisDrone.txt', 'a')
            record = open('VisDroneRecord.txt', 'a')
            if iteration % 10 == 0:
                print('iter ' + repr(iteration) + ' || Loss: %.4f ||' % (loss.data.item()), end='\n', file=output)
                print('Confidence loss: {:.2f}, Location loss: {:.2f}'.format(conf_loss.data.item(), loc_loss.data.item()), file=output)
                print('{},{},{},{}'.format(repr(iteration), loss.data.item(), conf_loss.data.item(), loc_loss.data.item(), end='\n'), file=record)
            output.close()
            record.close()
